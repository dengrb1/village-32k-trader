...
execute if score @s vt_ok matches 1 run function village_trader:main/task/give_3
execute if score @s vt_ok matches 1 run scoreboard players set @s vt_qrep 1
execute if score @s vt_ok matches 1 run title @s actionbar [{"text":"[村庄商人] 已免费补领一次「末地追踪证」。再次丢失将不再补领。","color":"aqua"}]

...
execute store result score @s vt_tmp run clear @s minecraft:prismarine_shard 0
execute if score @s vt_tmp matches ..15 run scoreboard players set @s vt_ok 0
execute store result score @s vt_tmp run clear @s minecraft:oak_boat 0
execute if score @s vt_tmp matches ..0 run scoreboard players set @s vt_ok 0
function village_trader:purchase/check_surcharge
execute if score @s vt_ok matches 1 run function village_trader:main/task/commit_4

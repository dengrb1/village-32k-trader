execute unless score @s vt_stage matches 3.. run title @s actionbar {"text":"[村庄商人] 尚未解锁 10 级通行证。","color":"red"}
execute unless score @s vt_stage matches 3.. run return 0
scoreboard players set @s vt_gear 2
execute if entity @s[tag=village_trader.main_pass_10] run function village_trader:main/equipment/open_pass
execute if entity @s[tag=village_trader.main_pass_10] run return 0
...
execute if score @s vt_ok matches 1 run tag @s add village_trader.main_pass_10
execute if score @s vt_ok matches 1 run advancement grant @s only village_trader:achievements/trade_03
execute if score @s vt_ok matches 1 run title @s actionbar {"text":"[村庄商人] 已购买 10 级装备通行证；现在可按需免费领取单件。","color":"green"}

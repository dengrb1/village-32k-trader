...
- Java：25。
- 插件文件：`VillageTrader-2.1.jar`（v2.1）。
...
构建产物位于 `build/libs/VillageTrader-2.1.jar`。
...
- `/vt progress`：在任意地点打开只读十章全局进度。
- `/vt buy <商品ID> <1-64>`：原子购买主线商品包；GUI 同时提供 1/16/64 包快捷按钮。
- 右键成长商人：打开个人商店。
...
| 章节 | 目标摘要 | 阶段奖励 |
|---|---|---|
| 1 定居启程 | 工作台、睡床、熔炼铁锭 | 面包、火把、5 级装备 |
| 2 矿脉勘探 | 8 钻石矿、20 敌对生物、红石矿 | 采掘补给、10 级装备 |
| 3 下界远征 | 进下界、10 烈焰人、4 残骸、猪灵 | 黑曜石、抗火 |
| 4 沧海巡航 | 远古守卫者、潮涌核心、海底神殿 | 潮汐补给、水下呼吸 |
| 5 末地远征 | 进末地、末影龙、末地城 | 末影补给、缓降、20 级装备 |
| 6 村庄守卫 | 赢得袭击、图腾、劫掠兽 | 守卫补给、32 级装备 |
| 7 试炼密室 | 3 旋风人、试炼钥匙、沼骸 | 试炼补给 |
| 8 凋灵攻坚 | 凋灵、信标、凋灵骷髅头 | 终战补给、64 级装备 |
| 9 深暗净化 | 监守者、8 回响碎片、远古城市 | 幽匿补给 |
| 10 龙魂再临 | 4 末地水晶、末影龙、龙息 | 终焉旗帜、255 级装备 |

完成第十章后才可切换困难终局。首次兑换等级时支付原有价格购买通行证，不自动塞满背包；按护甲、近战、工具、远程和功能装备分类，可无限次免费补领任意单件。主线通行证为 5/10/20/32/64/255 阶，儿童守护通行证为 1/3/5/10/20/32/128 阶。新发装备不再不可破坏，保留耐久、耐久附魔与经验修补。

主线商品为图标化逐项价格，新增煤炭、铁锭、铜锭、金锭、红石、青金石、下界石英、末影珍珠、火把、箭矢、熟牛排、六种药水、经验瓶、白色与紫色潜影盒等；每种商品按阶段和四档制裁等级计算实际单包价格。

主线辅助用品共有八件。建造者护符在主世界提供急迫 II 与缓降，旅行者护符提供速度与抗性。只能有一件被选中，且必须已登记并随身携带；商店显示四项状态，服务器只在状态发生变化时提示。

### 成就档案

商店首页的“成就档案”显示 40 项成就：剧情 10、探索 8、战斗 8、商会 7、守护 7。分类全满后可领取一次毕业补给；总数达到 5、15、25、35、40 时依次获得称号，40/40 额外获得“终焉传奇”旗帜。称号只在商店档案中显示，不修改聊天名。
...
4. 启动服务器，让玩家逐一登录。
5. 插件会在本地 JSON 不存在时读取旧计分板、标签和旧物品，生成一次性迁移档案。十章数据包会保留任务、装备、成就与领奖标签；旧六阶段数据包会映射为新章节，并把正在进行的旧任务转换为一次免费契约。
